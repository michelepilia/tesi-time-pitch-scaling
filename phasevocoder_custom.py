...
class PhaseVocoderConverter(Converter):
    ...
    def convert_frame(self, frame):
        #frame_no_unwrap = frame #Aggiunta Michele
        # pylint: disable=arguments-differ
        print("I'm inside convert_frame")
        for k in range(0, self._channels):
            print("I'm at frame " + str(k))
            # Compute the FFT of the analysis frame
            stft = np.fft.rfft(frame[k])
            amplitude = np.abs(stft)
            phase = np.angle(stft)
            #del stft #tolta michele
            ...
            if self._first:
                ...
                #stft_no_unwrap = stft #aggiunta michele
                frame[k, :] = np.fft.irfft(output_stft).real
                #frame_no_unwrap[k, :] = np.fft.irfft(stft_no_unwrap).real #aggiunta Michele
        ...
        return frame #, frame_no_unwrap #aggiunta Michele
